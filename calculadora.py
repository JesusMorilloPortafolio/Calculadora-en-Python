from tkinter import * #para los estilos tematicos
from tkinter import ttk
import math #para el apartado matematico (raiz cuadrada)


def TemaOscuro(*args):
    estilos.configure('mainframe.TFrame', background="#FFFFFF")

    estilos_label1.configure('Label1.TLabel', background="#010924", foreground="white")
    estilos_label1.configure('Label2.TLabel', background="#010924", foreground="white")
...
def TemaClaro(*args):
    estilos.configure('mainframe.TFrame', background="#000000", foreground="black")
   
    estilos_label1.configure('Label1.TLabel', background="#E9E8EA", foreground="black")
    estilos_label1.configure('Label2.TLabel', background="#E9E8EA", foreground="black")
    

    estilos_botones_numeros.configure('Botones_numeros.TButton', background="#FFFFFF", foreground="black")
    estilos_botones_numeros.map('Botones_numeros.TButton', background=[('active', '#808080')])

    estilos_botones_borrar.configure('Botones_borrar.TButton', background="#808080", foreground="black")
    estilos_botones_borrar.map('Botones_borrar.TButton', foreground=[('active', '#FF0000')], background=[('active', '#858585')])
    
    estilos_botones_restantes.configure('Botones_restantes.TButton', background="#808080", foreground="black")
    estilos_botones_restantes.map('Botones_restantes.TButton', background=[('active', '#FFFFFF')])


#FUNCION MATEMATICA DE LA CALCULADORA 
...
mensaje_bienvenida = "¡Bienvenido a la calculadora en Python de Jesus Morillo!"

ancho_pantalla = root.winfo_screenwidth()
alto_pantalla = root.winfo_screenheight()

# Calcular las coordenadas para centrar la ventana
x = (ancho_pantalla // 2) - (650 // 2)  # 600 es el ancho de la ventana
y = (alto_pantalla // 2) - (250 // 2)   # 200 es la altura de la ventana

# Configurar la posición de la ventana en el centro de la pantalla
root.geometry(f'650x250+{x}+{y}')
lbl_mensaje = tk.Label(root, text="", font=("Arial", 18))
lbl_mensaje.pack(pady=20)


lbl_mensaje = tk.Label(root, text=mensaje_bienvenida, font=("Arial", 18))
lbl_mensaje.pack(pady=5)

btn_ir_calculadora = tk.Button(root, text="Ir a la calculadora", command=cerrar_ventana,
                        width=45, height=2)  # Ajustar el ancho y alto del botón
btn_ir_calculadora.pack(padx=10, pady=10)
...
root = Tk()
root.title("Calculadora Jesus Morillo")
root.geometry("+500+80")#las cordenadas graficas de la interfaz de la calculadora
...
estilos.configure('mainframe.TFrame', background="#000000")
...
estilos_botones_numeros.configure('Botones_numeros.TButton', font="arial 22", width=5, relief="flat",  background="#FFFFFF")
estilos_botones_numeros.map('Botones_numeros.TButton', background=[('active', '#808080')])

estilos_botones_borrar = ttk.Style()
estilos_botones_borrar.configure('Botones_borrar.TButton', font="arial 22", width=5, relief="flat", background="#808080")
estilos_botones_borrar.map('Botones_borrar.TButton', foreground=[('active', '#FF0000')], background=[('active', '#858585')])

estilos_botones_restantes = ttk.Style()
estilos_botones_restantes.configure('Botones_restantes.TButton', font="arial 22", width=5, relief="flat", background="#808080")
estilos_botones_restantes.map('Botones_restantes.TButton', background=[('active', '#FFFFFF')])
...
Button_borrar = ttk.Button(mainframe, text=chr(9003), style="Botones_borrar.TButton", command=lambda: borrar()) #el metodo chr es para convertir el numero en un caracter 
...
#el MAINFRAME es para indicar al estilo donde se ubicaran los botones 
...
for child in mainframe.winfo_children():
    child.grid_configure(ipady=10, padx=1, pady=1) #esto maneja las separaciones 
